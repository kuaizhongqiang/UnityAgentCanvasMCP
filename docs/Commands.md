...
POST /cmd { "command": "result.show", "params": {
    "pageId": "page_1", "elementId": "elem_3",
...
### search.data {自然语言}
语义搜索数据。由 Embedding 引擎处理。返回 Top-N（含 knowledgeOriginal 锚点）。
...
### page.create {页面ID}
新建页面，返回 pageId。创建时为空白状态，初始布局使用 init 配置中的 `defaultLayout`。

### page.list
列出所有页面及其状态。

### run {页面ID}
执行已有页面，将页面配置渲染到 Unity UI。

### run {页面ID} {文件路径}
从 JSON 文件加载页面配置并渲染。filePath 相对于 `Application.streamingAssetsPath`。
...
### init {配置}
持久化到 `{persistentDataPath}/CLI/config.json`，重启/换 dialog 保留。

### restart
重启当前对话，清空状态回到初始。init 配置保留。
...
{
  "pageId": "page_1",
  "version": 1,              // Unity 自动自增，Agent 不管理。冲突时返回 409 Conflict
  ...
  "elements": [
    {
      ...
      "bind": "数据ID",
      "callback": true | false,
      "grading": "auto | agent",   // 仅 callback: true 时有效，缺省 = auto
