...
| `status_list` | `dialogId: string` | 所有请求状态 `[{requestId, command, status, data?}]` |
| `usage` | — | 统计数据 |
...
| 工具 | 参数 | 返回 |
|:--|:--|:--|
| `page_create` | `dialogId: string` | `{pageId, dialogId}` |
| `page_list` | `dialogId: string` | 页面列表 |
| `run` | `pageId: string, dialogId: string, filePath?: string` | requestId → 执行结果 |
| `update` | `pageId: string, dialogId: string, patch: object` | requestId → 执行结果 |
| `clear` | `pageId: string, dialogId: string, scope?: string` | requestId → 执行结果 |
| `result_show` | `pageId: string, dialogId: string, elementId: string, result: object` | requestId → 执行结果 |
| `page_delete` | `pageId: string, dialogId: string` | 确认 |
| `stop` | — | 确认 |
...
WebSocket 断开重连后，MCP Server 调用 `status_list {dialogId}` 查询所有请求状态：
